            ...
            {targetReport.type === 'video' && (
              <div className="space-y-5" id="video-evidence-preview">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 bg-slate-950 p-3 rounded-xl border border-slate-900">
                  {[
                    { frame: '00:02', title: 'Frame #048', desc: 'Specular reflections correct. Standard iris contours verified.', status: 'authentic', color: 'border-emerald-500/40 text-emerald-400' },
                    { frame: '00:08', title: 'Frame #192', desc: 'Normal jaw mesh locking verified. Face boundaries intact.', status: 'authentic', color: 'border-emerald-500/40 text-emerald-400' },
                    { frame: '00:14', title: 'Frame #336', desc: isFake ? 'SUSPICIOUS: 140ms lip audio delay. Mesh vertex jitter.' : 'Passed temporal cohesion test. Speech matches lips.', status: isFake ? 'suspicious' : 'authentic', color: isFake ? 'border-rose-500 text-rose-400 animate-pulse' : 'border-emerald-500/40 text-emerald-400' },
                    { frame: '00:22', title: 'Frame #528', desc: isFake ? 'SUSPICIOUS: Frame interpolation anomalies near cheek boundaries.' : 'Boundary pixels coherent with primary camera sensor.', status: isFake ? 'suspicious' : 'authentic', color: isFake ? 'border-rose-500 text-rose-400 animate-pulse' : 'border-emerald-500/40 text-emerald-400' },
                  ].map((f, i) => (
                    <div key={i} className={`bg-slate-900 rounded border p-2 text-center space-y-2 relative overflow-hidden flex flex-col justify-between ${
                      f.status === 'suspicious' ? 'ring-1 ring-rose-500/20' : ''
                    }`}>
                      <div className="absolute top-1 right-1 text-[8px] font-mono bg-slate-950 px-1 text-slate-400 rounded">
                        {f.frame}
                      </div>
                      
                      {/* Wireframe head representation */}
                      <div className="py-4 flex items-center justify-center relative">
                        {f.status === 'suspicious' && (
                          <div className="absolute inset-0 flex items-center justify-center bg-rose-500/5 pointer-events-none">
                            <span className="text-[10px] font-mono text-rose-500 border border-rose-500 px-1 font-bold rounded uppercase">ANOMALY</span>
                          </div>
                        )}
                        <svg className={`w-12 h-12 stroke-[0.8] fill-none ${f.status === 'suspicious' ? 'text-rose-500' : 'text-slate-600'}`}>
                          <circle cx="24" cy="20" r="14" className={f.status === 'suspicious' ? 'stroke-rose-500 stroke-[1.2]' : 'stroke-slate-700'} />
                          <ellipse cx="24" cy="22" rx="9" ry="12" />
                          <line x1="18" y1="18" x2="20" y2="18" className={f.status === 'suspicious' ? 'stroke-rose-500 stroke-2' : 'stroke-blue-400'} />
                          <line x1="28" y1="18" x2="30" y2="18" className={f.status === 'suspicious' ? 'stroke-rose-500 stroke-2' : 'stroke-blue-400'} />
                          <circle cx="24" cy="28" r="3" className={f.status === 'suspicious' ? 'stroke-rose-500 stroke-2 animate-ping' : 'stroke-blue-400'} />
                        </svg>
                      </div>

                      <div className="text-[9px] font-mono space-y-0.5">
                        <span className="block text-slate-400 uppercase font-bold">{f.title}</span>
                        <span className="block text-[8px] text-slate-500 h-8 overflow-hidden leading-tight text-center px-1">
                          {f.desc}
                        </span>
                        <span className={`block font-bold truncate mt-1 px-1 rounded border border-dashed text-[8px] ${f.color}`}>
